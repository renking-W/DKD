...
public class TaskController{
    ...
    public Pager<Task> search(
            @RequestParam(value = "pageIndex", required = false, defaultValue = "1") Long pageIndex,  //  页码
            @RequestParam(value = "pageSize", required = false, defaultValue = "10") Long pageSize,  //  每页数量
            @RequestParam(value = "innerCode", required = false, defaultValue = "") String innerCode, //  设备编号
            @RequestParam(value = "userId", required = false, defaultValue = "") Integer userId, //  工单所属人Id
            @RequestParam(value = "taskCode", required = false, defaultValue = "") String taskCode, //  工单编号
            @RequestParam(value = "status", required = false, defaultValue = "") Integer status, //  工单状态
            @RequestParam(value = "isRepair", required = false, defaultValue = "") Boolean isRepair, //  是否是维修工单
            @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate start,  //  开始时间
